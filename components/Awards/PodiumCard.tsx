...
import { Trophy, Medal, Gift, Ticket, DollarSign } from "lucide-react";
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-3xl transition-all duration-500 cursor-pointer ${
        isActive
          ? `bg-gradient-to-br ${award.bgColor} shadow-2xl scale-110 border-2 border-white/50`
          : "bg-white/90 backdrop-blur-xl shadow-xl hover:shadow-2xl border border-white/20"
      } ${award.shadow}`}
    ...
    >
      ...
      <div className='relative p-8 text-center'>
        {/* Icono principal */}
        <div
          className={`mx-auto mb-6 p-4 rounded-full transition-all duration-500 ${
            isActive
              ? `bg-gradient-to-br ${award.color} shadow-xl`
              : "bg-gray-100"
          }`}
        >
          <IconComponent
        ...
        {/* Contenido */}
        <h3
          className={`text-2xl lg:text-3xl font-bold mb-3 transition-colors duration-500 ${
...
function CategoryCard({ category, index, isActive, onHover, onLeave }) {
