  ...
  return (
    <div className="field">
      <div className="control">
        ...
        <input
          className={`block bg-white w-full border border-slate-300 rounded-md py-2 px-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm ${meta.touched && meta.error && 'is-danger'}`}
          {...field}
          {...props}
          autoComplete="off"
        />
...
export const ButtonField = ({ type, addText, classType }) => {
  return (
    <button type={type} class={`button ${classType}`}>
      {addText}
    </button>
  )
}
