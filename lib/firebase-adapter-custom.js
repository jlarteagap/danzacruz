  ...
  return {
    async createUser(user) {
      ...
      const userData = {
        ...
        id: userRef.id,
        role: user.role || "user",
        profileComplete: user.profileComplete || false, // Nuevo campo
        createdAt: new Date(),
    ...
    async getUser(id) {
      ...
      return {
        ...
        profileComplete: userData.profileComplete || false, // Asegurar que existe
      };
    },
    ...
    async updateSession({ sessionToken, expires }) {
      ...
      const db = await getDb();
      const sessionRef = db.collection("sessions").doc(sessionToken); // CORRECCIÓN: era userRef

      await sessionRef.update({ expires }); // CORRECCIÓN: era userRef.update
